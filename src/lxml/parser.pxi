...
# NOTE: the '%s' in this message is filled in by the error reporting machinery
# from the 'str1' field of the error struct.  Keep it intact.
cdef const_char* _EXTERNAL_ENTITY_ERROR_MESSAGE = (
...
cdef tree.xmlEntity* _getInternalEntityOnly(void* ctxt, const_xmlChar* name) nogil:
    # Callback function to intercept the entity resolution
    # when external entity loading is disabled.
    ...
    if not entity:
        return NULL
    if (entity.etype != tree.XML_EXTERNAL_GENERAL_PARSED_ENTITY and
            entity.etype != tree.XML_EXTERNAL_GENERAL_UNPARSED_ENTITY and
            entity.etype != tree.XML_EXTERNAL_PARAMETER_ENTITY):
        return entity
    ...
    if err_func is not NULL:
        ...
        if parser_input is not NULL and parser_input.filename is NULL and c_ctxt.inputNr > 1:
        ...
        c_error.message = <char*> _EXTERNAL_ENTITY_ERROR_MESSAGE
        c_error.file = (<char*> parser_input.filename) if parser_input is not NULL else NULL
        c_error.node = <void*> entity
        c_error.str1 = <char*> name
        c_error.str2 = NULL
        c_error.str3 = NULL
        c_error.line = parser_input.line if parser_input is not NULL else 0
        c_error.int1 = 0
        c_error.int2 = parser_input.col if parser_input is not NULL else 0
